...
此命令會執行自訂單元測試架構，以便在專案中Asset compute應用程式動作，如下所述。 它會透過 `package.json` 檔案。 也可以有JavaScript單元測試，例如Jest。 此 `aio app test` 執行兩者。

此 [aio-cli-plugin-asset-compute](https://github.com/adobe/aio-cli-plugin-asset-compute#install-as-local-devdependency) 外掛程式會嵌入為自訂應用程式中的開發相依性，因此不需要安裝在建置/測試系統上。
...
測試應在 `test` 位於專案根層級的資料夾。 每個應用程式的測試案例應該位於路徑中 `test/asset-compute/<worker-name>`，每個測試案例各有一個資料夾：
...
此 `build` 位於Adobe Developer App Builder應用程式根目錄的區域，內含自訂應用程式的詳細測試結果和記錄。 這些詳細資料也會顯示在 `aio app test` 命令。

### 模擬外部服務 {#mock-external-services}

您可以建立 `mock-<HOST_NAME>.json` 測試案例的檔案，其中HOST_NAME是您要模擬的特定主機。 使用案例範例是對S3進行個別呼叫的應用程式。 新的測試結構如下所示：
...
Adobe建議使用相對符號連結（若您共用） `file.*`， `params.json` 或 `validate` 跨多個測試的指令碼。 它們受到Git支援。 請務必為您共用的檔案指定唯一的名稱，因為您可能有不同的檔案。 在以下範例中，測試會將一些共用檔案與其自己的檔案混合及比對：
...
>[!NOTE]
>
>如果測試案例未包含預期的 `rendition.*` 且未定義預期的 `errorReason` 內部 `params.json` 檔案中，若出現下列任何錯誤，則視為錯誤案例 `errorReason`.
...
檢視完整清單和說明 [asset compute錯誤原因](https://github.com/adobe/asset-compute-commons#error-reasons).

## 對自訂應用程式進行偵錯 {#debug-custom-worker}

下列步驟顯示如何使用Visual Studio Code偵錯自訂應用程式。 它允許檢視即時記錄、點選中斷點和逐步執行程式碼，以及在每次啟用時即時重新載入本機程式碼變更。

此 `aio` 現成可用的自動化了許多步驟。 前往中偵錯應用程式區段 [Adobe Developer App Builder檔案](https://developer.adobe.com/app-builder/docs/getting_started/first_app). 目前，以下步驟包含因應措施。

1. 安裝最新版本 [wskdebug](https://github.com/apache/openwhisk-wskdebug) 來自GitHub和選購的 [Ngrok](https://www.npmjs.com/package/ngrok).
...
1. 在JSON檔案中新增使用者設定。 它持續使用舊的Visual Studio程式碼偵錯工具。 新版本具有 [部分問題](https://github.com/apache/openwhisk-wskdebug/issues/74) 使用wskdebug： `"debug.javascript.usePreview": false`.
1. 關閉透過以下方式開啟的任何應用程式例項： `aio app run`.
1. 使用部署最新的程式碼 `aio app deploy`.
1. 只執行Asset computeDevtool，使用 `aio asset-compute devtool`. 保持開啟。
...
1. 選取 `wskdebug worker` 從「執行/偵錯」設定，然後按「播放」圖示。 等待它開始，直到它顯示 **[!UICONTROL 準備啟用]** 在 **[!UICONTROL 除錯主控台]** 視窗。

1. 按一下 **[!UICONTROL 執行]** 在Devtool中。 您可以看到在Visual Studio程式碼編輯器中執行的動作，而且記錄會開始顯示。
